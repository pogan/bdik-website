...
const MAX_AMOUNT = 200000; // 2000,00 zł - sufit; powyżej i tak jest to "cała baza"

const TIERS = [
  { upTo: 100, perRow: 100 }, // 1 zł / rekord
  { upTo: 1000, perRow: 80 },
  { upTo: 10000, perRow: 60 },
  { upTo: Infinity, perRow: 50 },
];
...
// Nazwa pozycji widoczna na stronie płatności Stripe i na paragonie. Gdy część
// rekordów jest bezpłatna (brak danych kontaktowych), mówimy to wprost - klient
// widzi na paragonie, że płaci tylko za rekordy z kontaktem.
function lineItemName(format, rowCount, billedCount = null) {
  const base = `Eksport ${FORMAT_LABELS[format] || format.toUpperCase()} - ${rowCount} instytucji kultury`;
  if (billedCount === null || billedCount >= rowCount) return base;
  return `${base} (płatne ${billedCount} z danymi kontaktowymi, ${rowCount - billedCount} gratis)`;
}

// Czytelny opis wybranych filtrów ("woj. pomorskie · pow. gdański · \"biblioteka\"").
const CONTACT_FILTER_LABELS = {
  any: 'tylko z danymi kontaktowymi',
  phone: 'tylko z telefonem',
  email: 'tylko z e-mailem',
  website: 'tylko ze stroną WWW',
};
...
function describeFilters(filters = {}) {
  ...
  if (filters.contact && CONTACT_FILTER_LABELS[filters.contact]) parts.push(CONTACT_FILTER_LABELS[filters.contact]);
