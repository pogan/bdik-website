...
// Komplet kluczy rozumianych przez buildWhere (lib/query.js) - dzięki temu
// eksport administratora, który filtruje po kolumnach spoza publicznych czterech,
// odtworzy dokładnie ten sam zakres, jaki widział w tabeli. Publiczne UI wysyła
// tylko podzbiór, więc dla klientów nic się nie zmienia.
const { ADMIN_FILTERABLE_COLUMNS } = require('./query');

// 'contact' to publiczny filtr "tylko z danymi kontaktowymi" - wartość i tak
// przechodzi przez białą listę warunków w buildWhere (lib/query.js).
const FILTER_KEYS = [...ADMIN_FILTERABLE_COLUMNS, 'contact'];

const insertOrder = db.prepare(`
  INSERT INTO orders (
    token, user_id, email, format, filters, row_count, billed_count, created_by_admin,
    amount, currency, status, terms_version, terms_accepted_at, withdrawal_consent_at
  )
  VALUES (
    @token, @user_id, @email, @format, @filters, @row_count, @billed_count, @created_by_admin,
    @amount, @currency, 'pending', @terms_version, datetime('now'), datetime('now')
  )
`);
...
function createOrder({ userId = null, email = null, format, selection, rowCount, billedCount = null, createdByAdmin = false, amount, currency, termsVersion = null }) {
